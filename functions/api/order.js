...
const MOTHERS_PACKAGES = {
  "Box 1": 12.50, "Box 2": 25.00, "Box 3": 30.00, "Box 4": 35.00,
  "Bouquet Bloom": 12.50, "Garden Party": 25.00, "Full Bloom": 30.00, "Mum's Treasure": 35.00
};

const FRAME_PRICES = {
  bohemian: { 3: { frame: 8, full: 15 }, 4: { frame: 10, full: 17 }, 6: { frame: 12, full: 25 }, 9: { frame: 15, full: 30 } },
  sleek: { 1: { frame: 5, full: 7 }, 2: { frame: 7, full: 12 }, 3: { frame: 8, full: 15 }, 4: { frame: 10, full: 17 } },
  rollercube: { 4: { frame: 10, full: 17 } }
};
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    let price = 0;
    // FIX: Prevent undefined pack sizes from turning into 'null'
    const packSize = packSizeRaw ? Number(packSizeRaw) : null;

    if (eventTag === 'MANUAL') { price = 0; } 
    else if (String(packSizeRaw).startsWith("voucher_")) { price = 0; } 
    else if (eventTag === 'MOTHERS_DAY') {
        if (productType === 'frames') {
            const sizeData = FRAME_PRICES[frameStyle]?.[frameSize];
        ...
        if (eventTag === 'FRAMES') {
            ...
            if (productType === 'flexi') price = FLEXI_PRICE;
            else {
    ...
    // FIX: The Anti-Downgrade Lock. 
    // Once an order is paid, a rogue delayed API call can never force it back to draft.
    const protectedStatuses = ['paid', 'printing', 'shipped', 'completed'];
    let finalStatus = "draft";
    if (protectedStatuses.includes(existingOrder.status)) {
        finalStatus = existingOrder.status; 
    } else if (manualStatus === 'paid') {
        finalStatus = 'paid';
    } else {
        finalStatus = existingOrder.status || "draft";
    }

    const now = new Date().toISOString();
    let socialPerm = existingOrder.socialPermission;
    if (body.socialPermission !== undefined) socialPerm = body.socialPermission;

    const order = {
      orderId, email, phone, packSize, price, event: eventTag, raffleNumber, 
      productType, flexiColor, premadeSelections, mothersPackage,
      frameStyle, frameSize, frameColor, includeMagnets,
      status: finalStatus, 
